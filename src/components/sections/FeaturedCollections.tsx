    ...
    <section className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">Discover our signature collections that blend traditional Ulos craftsmanship with contemporary design</p>
        </div>
          ...
          {collections?.slice(0, 3)?.map((collection) => (
            <div key={collection.id} className="bg-white rounded-lg shadow-lg overflow-hidden group hover:shadow-xl transition-shadow">
              <div className="relative h-64 bg-gradient-to-br from-gray-200 to-gray-300">
                ...
                <div className={`absolute inset-0 flex items-center justify-center ${collection.coverImage || collection.image ? "hidden" : "flex"}`}>
                  <span className="text-gray-500 text-lg font-medium">{collection.name}</span>
                </div>
                <div className="absolute top-4 right-4 bg-[#921e27] text-white px-3 py-1 rounded-full text-sm">{collection.products?.length || 0} Products</div>
              </div>
        ...
        {/* Featured Products */}
        {collections?.some((collection) => collection.products?.some((product) => product.featured)) && (
          <div className="mb-16">
                    ...
                    <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden group hover:shadow-lg transition-shadow">
                      <div className="relative h-48 bg-gradient-to-br from-gray-200 to-gray-300">
                        {product.images && product.images.length > 0 ? (
                          <Image src={product.images[0]} alt={product.name} fill className="object-cover group-hover:scale-105 transition-transform" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                        ) : null}
        ...
        <div className="text-center">
          <Link href="/portfolio" className="inline-flex items-center bg-black text-white px-8 py-3 rounded-lg hover:bg-gray-800 transition-colors font-medium">
